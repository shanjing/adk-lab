...
async def dump_event(event):
    ...
    logger.info(f"Event Content:\n{json.dumps(event.content, indent=2, default=str)}")
    logger.info("=" * 51)
    ...
    if event.is_final_response():
        if (
            ...
            logger.info(f"==> Final Agent Response: [No text content in final event]")
...
def reconstruct_state(event_log: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for event in event_log:
        ...
        delta: Dict[str, Any] = event.get("state_delta") or event.get(
            "payload", {}
        ).get("state_delta")
